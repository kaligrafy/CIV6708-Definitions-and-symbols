...
\section*{Transport collectif: définitions et symboles • \textit{Public transit: definitions and symbols}}

\subsection*{Arrêts, noeuds d'arrêt, stations et segments • \textit{Stops, stop nodes, stations and segments}}
...
\hline
\makecell[r]{Station \\ \textit{Station}} & \[S\] & - & - & Bâtiment de service de grande taille facilitant le transfert entre plusieurs arrêts. Lorsque plusieurs modes distincts se trouvent à proximité ou à l'intérieur de la station, on l'appelle alors station intermodale. \\
\hline
\makecell[r]{Segment \\ \textit{Segment}} & \[l\] & - & - & Segment de ligne entre deux arrêts, représenté par un trajet précis. \\
\hline
...
\hline
\makecell[r]{Terminus \\ \textit{Terminus}} & \[s_T\] ou \[q_T\]  & - & - & Premier ou dernier noeud d'arrêt ou arrêt d'une ligne. Permet habituellement à au moins une unité de se stationner pour le temps de battement. \\
\hline
\makecell[r]{Terminus de départ \\ \textit{Departure terminus}} & \[{s^{\prime}_T}\] ou \[q^{\prime}_T\] & - & - & Premier noeud d'arrêt ou premier arrêt d'un parcours ou d'une ligne. \\
\hline
\makecell[r]{Terminus d'arrivée  \\ \textit{Arrival terminus}} & \[{s^{\prime\prime}_T}\] ou \[{q^{\prime\prime}_T}\] & - & - & Dernier noeud d'arrêt ou dernier arrêt d'un parcours ou d'une ligne. \\
\hline
...
\hline
\makecell[r]{Arrêt-séquence \\ \textit{Stop-sequence}} & \[s_{seq}\] ou \[q_{seq}\] & - & - & Combinaison d'un noeud d'arrêt ou d'arrêt et d'un numéro de séquence sur un parcours d'une ligne. Un parcours comprend un arrêt-séquence pour chaque arrêt desservi. \\
\hline
...
\hline
\makecell[r]{Taille de la flotte en service \\ \textit{Service fleet size}} & \[F_s\] & unités & - & Nombre d'unités pouvant être utilisées pour effectuer le service planifié pendant la période de pointe maximale. \\
\hline
...
\hline
\makecell[r]{Accélération programmée \\ \textit{Programmed acceleration}} & \[a_p\] & \[m/s^2\] & - & Accélération utilisée en service. Cette accélération doit être le meilleur équilibre entre le confort des passagers, l'efficacité énergétique et l'optimisation du service. \\
\hline
...
\hline
\makecell[r]{Décélération programmée \\ \textit{Programmed deceleration}} & \[b_p\] & \[m/s^2\] & - & Décélération utilisée en service. Cette décélération/freinage doit être le meilleur équilibre entre le confort des passagers et l'optimisation du service. Utiliser en valeur absolue (positive). \\
\hline
\end{longtable}
...
\hline
\makecell[r]{Parcours \\ \textit{Path}} & \[p\] & - & - & Trajet unique dans une direction déterminée sur une ligne. Un parcours est défini pour chaque séquence d'arrêts distincte desservie par la ligne. Un parcours comprend un ensemble d'arrêts-séquences et un trajet géographique sur le réseau (routier, ferroviaire ou autre). \\
\hline
...
\hline
\makecell[r]{Directivité spatiale \\ \textit{Spatial directivity}} & \[\delta\] & - & \[\frac{d_e}{d_o}\] & Ratio représentant le caractère direct d'un parcours par rapport au trajet direct à vol d'oiseau (distance euclidienne). Inverse de la tortuosité (aussi appelée directitude). \\
\hline
\makecell[r]{Tortuosité spatiale \\ \textit{Tortuosity}} & \[\tau\] & - & \[\frac{d_o}{d_e}\] & Ratio représentant le caractère tortueux du parcours par rapport au trajet direct à vol d'oiseau (distance euclidienne). Inverse de la directivité. \\
\hline
\makecell[r]{Directivité spatiale réseau \\ \textit{Spatial network directivity}} & \[\delta_n\] & - & \[\frac{d_n}{d_o}\] & Ratio représentant le caractère direct d'un parcours par rapport au trajet le plus court en distance sur le réseau entre les deux terminus (habituellement le parcours le plus court en voiture). Inverse de la tortuosité réseau. \\
\hline
\makecell[r]{Tortuosité spatiale réseau \\ \textit{Spatial network tortuosity}} & \[\tau_n\] & - & \[\frac{d_o}{d_n}\] & Ratio représentant le caractère tortueux du parcours par rapport au trajet le plus court en distance sur le réseau entre les deux terminus (habituellement le trajet le plus court en voiture). Inverse de la directivité réseau. \\
\hline
\makecell[r]{Directivité temporelle \\ \textit{Temporal Directivity}} & \[\delta_t\] & - & \[\frac{t_n}{t_o}\] & Ratio représentant le caractère direct d'un parcours par rapport au trajet le plus rapide en temps sur le réseau entre les deux terminus (habituellement le parcours le plus rapide en voiture). Inverse de la tortuosité temporelle. \\
\hline
\makecell[r]{Tortuosité temporelle \\ \textit{Temporal tortuosity}} & \[\tau_t\] & - & \[\frac{t_o}{t_n}\] & Ratio représentant le caractère tortueux du parcours par rapport au trajet le plus rapide en temps sur le réseau entre les deux terminus (habituellement le trajet le plus direct en voiture). Inverse de la directivité temporelle. \\
\hline
\end{longtable} 
...
\hline
\endhead
\makecell[r]{Distance d'accélération \\ \textit{Acceleration distance}} & \[d_a\] & \[m\] & \[\frac{{v_p}^2}{2 a_p}\] & Distance parcourue lors de la phase d'accélération après avoir quitté l'arrêt précédent. \\
\hline
\makecell[r]{Distance de décélération \\ \textit{Deceleration distance}} & \[d_b\] & \[m\] & \[\frac{{v_p}^2}{2 b_p}\] & Distance parcourue lors de la phase de décélération/freinage avant d'atteindre le prochain arrêt. \\
\hline
...
\hline
\makecell[r]{Distance d'accès à l'origine \\ \textit{Access distance}} & \[{d_a}_O\] & \[m\] & - & Distance parcourue par l'usager de l'origine du déplacement au premier arrêt de son trajet de transport collectif. \\
\hline
\makecell[r]{Distance d'accès à destination \\ \textit{Egress distance}} & \[{d_a}_D\] & \[m\] & - & Distance parcourue par l'usager du dernier arrêt de son trajet de transport collectif à la destination du déplacement. \\
\hline
\makecell[r]{Distance d'accès à l'origine et à destination \\ \textit{Access/egress distance}} & \[{d_a}_{OD}\] & \[m\] & \[{d_a}_O + {d_a}_D\] & Distance parcourue par l'usager de l'origine au premier arrêt et du dernier arrêt à la destination de son trajet de transport collectif. \\
\hline
...
\hline
\makecell[r]{Distance totale de transfert \\ \textit{Total transfer distance}} & \[{d_{tr}}_{tot}\] & \[m\] & \[\sum_{i=1}^{n_{tr}} {d_{tr}}_i\] & Distance totale parcourue pour se déplacer d'un arrêt à l'autre lors de tous les transferts effectués pendant le trajet. N'inclut pas les distances d'accès à l'origine et à destination. \\
\hline
\makecell[r]{Distance totale d'accès et de transfert \\ \textit{Total access/sgress and transfer distance}} & \[d_{a_{tot}}\] & \[m\] & \[{d_a}_{OD} + {d_{tr}}_{tot}\]& Somme des distances d'accès de l'origine du déplacement au premier arrêt et du dernier arrêt de son trajet de transport collectif à la destination du déplacement, et de la distance totale de transfert. Cette distance représente la distance totale parcourue à pied, à vélo ou en voiture pour accéder au service de transport collectif à l'origine et à destination et pour se déplacer entre les arrêts de transfert. \\
\hline
...
\hline
\makecell[r]{Distance totale en véhicule \\ \textit{Total in-vehicle distance}} & \[d_{{veh}_{tot}}\] & \[m\] & \[\sum_{i=1}^{n_{tr}+1} d_{{veh}_i}\] & Distance totale parcourue en véhicule lors du trajet de transport collectif complet, incluant les distances de tous les segments de ligne empruntés. \\
\hline
\makecell[r]{Distance totale OD \\ \textit{Total OD distance}} & \[d_{OD}\] & \[m\] & \[d_{a_{tot}} + d_{{veh}_{tot}}\] & Distance totale parcourue lors d'un déplacement de transport collectif, incluant l'accès et les transferts. \\
\hline
...
\hline
\makecell[r]{Distance totale OD voiture \\ \textit{Total OD driving distance}} & \[d_{car}\] ou \[d_{driving}\]  & \[m\] & - & Distance totale du chemin le plus rapide calculé en voiture. Attention de bien spécifier si le trajet optimal a été calculé en tenant compte de la congestion. \\
\hline
\makecell[r]{Distance totale OD vélo \\ \textit{Total OD cycling distance}} & \[d_{bicycle}\] ou \[d_{cycling}\] & \[m\] & - & Temps total du chemin le plus rapide calculé à vélo. \\
\hline
...
\hline
\endhead
\makecell[r]{Temps d'accélération \\ \textit{Acceleration time}} & \[t_a\] & \[s\] & \[\frac{v_p}{a_p}\] & Temps requis pour accélérer de l'arrêt complet à la vitesse programmée. \\
\hline
\makecell[r]{Temps de décélération \\ \textit{Deceleration time}} & \[t_b\] & \[s\] & \[\frac{v_p}{b_p}\] & Temps requis pour ralentir et freiner de la vitesse programmée à l'arrêt complet. \\
\hline
...
\hline
\makecell[r]{Temps de parcours de segment médian \\ \textit{Median interstop travel time}} & \[\widetilde{t_l}\] & \[s\] & \[\frac{\sum_{i=1}^{n_l} {t_l}_i}{n_l}\] & Médiane des temps de parcours inter-arrêt d'un parcours ou des parcours aller et retour d'une ligne bidirectionnelle. \\
\hline
...
\hline
\makecell[r]{Temps d'opération \\ \textit{Operating time}} & \[T_o\] & \[s\] & \[\sum_{i=1}^{n_l} {t_l}_i + \sum_{i=2}^{n_q - 1} {t_q}_i \] & Temps total entre le départ du terminus de départ et l'arrivée au terminus d'arrivée. Ne pas inclure les temps d'arrêt aux terminus. \\
\hline
\makecell[r]{Temps d'opération aller \\ \textit{Outbound operating time}} & \[{T_o}^\prime\] & \[s\] & \[\sum_{i=1}^{{n_l}_p} {t_l}_i + \sum_{i=2}^{n_q - 1} {t_q}_i \] où p est le parcours aller & Temps total entre le départ du terminus de départ et l'arrivée au terminus d'arrivée du parcours aller. Ne pas inclure les temps d'arrêt aux terminus. \\
\hline
\makecell[r]{Temps d'opération retour \\ \textit{Inbound operating time}} & \[{T_o}^{\prime\prime}\] & \[s\] & \[\sum_{i=1}^{{n_l}_p} {t_l}_i + \sum_{i=2}^{n_q - 1} {t_q}_i \] où p est le parcours retour & Temps total entre le départ du terminus de départ et l'arrivée au terminus d'arrivée du parcours retour. Ne pas inclure les temps d'arrêt aux terminus. \\
\hline
\makecell[r]{Temps de battement \\ \textit{Layover time}} & \[{t_t}\] & \[s\] & - & Temps requis pour débarquer les passagers au dernier arrêt, pour permettre à l'unité de se remettre en disponibilité pour le parcours suivant, pour rattraper les retards accumulés et pour embarquer les passagers pour le prochain départ en direction inverse. \\
\hline
...
\hline
\makecell[r]{Temps d'accès \\ \textit{Access/egress time}} & \[t_{a}\] & \[s\] & - & Temps de parcours à pied, à vélo ou en voiture permettant d'atteindre un arrêt. \\
\hline
\makecell[r]{Temps d'accès à l'origine \\ \textit{Access time}} & \[{t_a}_O\] & \[s\] & \[v_a {d_a}_O\] & Temps d'accès de l'usager entre l'origine du déplacement et le premier arrêt de son trajet de transport collectif. Un calculateur de chemin réseau pour le mode d'accès (marche, vélo, voiture ou autre) peut permettre d'obtenir un temps d'accès calibré et tenant compte des obstacles et des feux de circulation. \\
\hline
\makecell[r]{Temps d'accès à destination \\ \textit{Egress time}} & \[{t_a}_D\] & \[s\] & \[v_a {d_a}_D\] & Temps d'accès de l'usager entre le dernier arrêt de son trajet de transport collectif et la destination du déplacement. \\
\hline
\makecell[r]{Temps total d'accès à l'origine et à destination \\ \textit{Access/egress time}} & \[{t_a}_{OD}\] & \[s\] & \[{t_a}_O + {t_a}_D\]& Temps total d'accès de l'usager de l'origine du déplacement au premier arrêt et du dernier arrêt de son trajet de transport collectif à la destination du déplacement. N'inclut pas le temps total d'accès de transfert. \\
\hline
...
\hline
\makecell[r]{Temps total en véhicule \\ \textit{Total in-vehicle time}} & \[{t_{veh}}_{tot}\] & \[s\] & \[\sum_{i=1}^{n_{tr}+1} t_{{veh}_i}\] & Temps total passé en véhicule lors du trajet de transport collectif complet, incluant les temps de parcours de tous les segments de ligne empruntés et les temps d'arrêt en route. \\
\hline
\makecell[r]{Temps total OD \\ \textit{Total OD time}} & \[T_{OD}\] & \[s\] & \[ {t_a}_{tot} + {t_w}_{tot} + {t_{veh}}_{tot}\] & Temps total entre l'origine et la destination. \\
\hline
\makecell[r]{Temps moyen d'embarquement par passager \\ \textit{Average single boarding time}} & \[\overline{t_B}\] & \[s\] & - & \\
\hline
\makecell[r]{Temps moyen de débarquement par passager \\ \textit{Average single aligthing time}} & \[\overline{t_A}\] & \[s\] & - & \\
\hline
...
\hline
\makecell[r]{Coefficient d'efficacité globale \\ \textit{Global efficiency coefficient}} & \[\eta\] \[{\eta}_L\] & - & \[\eta_a \eta_s \eta_t\] \[{\eta_a}_L {\eta_s}_L {\eta_t}_L\] & Représente l'efficacité globale d'une ligne ou d'un réseau. Utiliser l'indice L lorsque le coefficient représente l'efficiacité d'une ligne en particulier (la ligne ne doit pas partager d'unités ou de main d'oeuvre avec une autre). \\
\hline
...
\hline
\makecell[r]{Temps total OD en voiture \\ \textit{Total driving OD travel time}} & \[T_{car}\] ou \[T_{driving}\] & \[s\] & - & Temps de parcours du chemin le plus rapide calculé en voiture. Attention de bien spécifier si ce temps et le trajet emprunté tient compte de la congestion. \\
\hline
...
\hline
\makecell[r]{Vitesse d'accès \\ \textit{Access/egress speed}} & \[v_a\] & \[{km}/h\] & - & Vitesse de marche, à vélo ou en voiture pour accéder aux arrêts à l'origine et à destination et lors des transferts. Un calcul de chemin bien calibré permet d'obtenir des vitesses plus réalistes, en tenant compte notamment des feux de circulation, de la congestion et des obstacles. \\
\hline
\makecell[r]{Vitesse OD \\ \textit{OD speed}} & \[V_{OD}\] & \[{km}/h\] & \[\frac{d_{OD}}{T_{OD}}\] & Vitesse moyenne globale d'un trajet de l'origine à la destination, en tenant compte des accès, des transferts, de tous les segments en véhicule, ainsi que de tous les temps d'arrêt et d'attente. \\
\hline
\end{longtable}
...
\hline
\endhead
\makecell[r]{Fréquence \\ \textit{Frequency}} & \[f\] & \[\text{unités}/h\] & \[\frac{60}{h}=\frac{n_u}{t}\] & Nombre de passages d'une unité par heure sur un parcours ou une ligne (t est en heures). \\
\hline
...
\hline
\makecell[r]{Intervalle \\ \textit{Period}} & \[h\] & \[min\] & \[\frac{60}{f}=\frac{t}{n_u}\] & Intervalle de temps entre le passage consécutif de deux unités sur un parcours ou une ligne (t est en minutes). \\
\hline
\makecell[r]{Intervalle maximum \\ \textit{Maximum period}} & \[h_{max}\] & \[min\] & \[\frac{60}{f_{min}}\] & Intervalle de service maximum selon contraintes et réglementations nationales, régionales ou locales. Cet intervalle peut être différente selon la période de service (matin, pointe du matin, journée, pointe du soir, soir, nuit, fin de semaine, etc.) \\
\hline
\makecell[r]{Intervalle maximum de pointe \\ \textit{Maximum peak period}} & \[{f_{peak}}_{max}\] & \[\text{unités}/h\] & \[\frac{60}{{h_{peak}}_{min}}\] & Intervalle maximum de pointe selon contraintes et réglementations nationales, régionales ou locales. \\
\hline
\makecell[r]{Intervalle maximum hors-pointe \\ \textit{Maximum off-peak period}} & \[{h_{off}}_{max}\] & \[\text{min}\] & \[\frac{60}{{f_{off}}_{min}}\] & Période maximum hors-pointe selon contraintes et réglementations nationales, régionales ou locales. Habituellement valable entre les pointes du matin et de l'après-midi et en soirée. \\
\hline
\makecell[r]{Intervalle minimum selon contrainte de voie \\ \textit{Way constrained minimum period}} & \[{h_l}_{min}\] & \[min\] & \[\frac{60}{{f_l}_{max}}\] & Intervalle minimum de service possible en tenant compte de la voie dans laquelle se déplace les unités affectées au parcours ou à la ligne. Cet intervalle tient compte des vitesses maximales permises, des critères de sécurité et des obstacles sur la voie (aiguillages, congestion, virages, etc.) \\
\hline
\makecell[r]{Intervalle minimum selon contrainte d'arrêt \\ \textit{Dwell constrained minimum period}} & \[{h_s}_{min}\] & \[min\] & \[\frac{60}{{f_s}_{max}}\] & Intervalle minimum de service possible en tenant compte de l'arrêt limitant sur le parcours ou la ligne. Tient compte de l'espace disponible aux arrêts, du nombre d'unités pouvant s'y arrêter, des temps d'arrêts prévus et des critères de sécurité. La plupart du temps, \({h_s}_{min} \gg {{h_l}_{min}}\) \\
\hline
\makecell[r]{Intervalle minimum \\ \textit{Minimum period}} & \[h_{min}\] & \[min\] & \[\max \Big({{h_s}_{min}}\ ,\ {{h_l}_{min}}\Big)\] & Fréquence maximale de service possible en tenant compte des contraintes de voie et d'arrêt. \\
\hline
\makecell[r]{Intervalle de pointe \\ \textit{Peak period}} & \[h_{peak}\] & \[min\] & \[\frac{60}{f_{peak}}\] & Intervalle pendant la période de pointe maximale (intervalle minimum offert) sur un parcours ou une ligne. \\
\hline
\makecell[r]{Intervalle d'ultra-pointe \\ \textit{Ultra-peak period}} & \[h_{up}\] & \[min\] & \[\frac{60}{f_{up}}\] & Intervalle requis pendant le 15 minutes le plus achalandé de la période de pointe maximale (intervalle minimum requis) sur un parcours ou une ligne. \\
\hline
\makecell[r]{Intervalle hors-pointe \\ \textit{Off-peak period}} & \[h_{off}\] & \[min\] & \[\frac{60}{f_{off}}\] & Intervalle pendant la période hors-pointe (fréquence offerte) sur un parcours ou une ligne. \\
\hline
\makecell[r]{Intervalle moyen \\ \textit{Average period}} & \[\overline{h}\] & \[min\] & \[\frac{\sum_{i=1}^{n_\text{périodes}} {h_i}}{n_\text{périodes}}\] & Intervalle moyen d'un ensemble de périodes de service sur un parcours ou une ligne. \\
\hline
\makecell[r]{Intervalle de pointe pondéré par arrêt \\ \textit{Stop weighted peak period}} & \[{h_{peak}}_s\] ou \[{h_{peak}}_q\] & \[min\] & \[\frac{\sum_{i=1}^{n_q} {h_i}}{n_q}\] & Attention de spécifier si l'intervalle à chaque arrêt est l'intervalle de la ligne avec le plus petit intervalle qui dessert l'arrêt ou l'intervalle moyen de toutes les lignes qui desservent l'arrêt. \\
\hline
\makecell[r]{Intervalle de pulsation \\ \textit{Pulsation period}} & \[h_p\] & \[min\] & - & Intervalle permettant de synchroniser plusieurs lignes entre elles aux points focaux (horaires cadencés ou \textit{timed-transfer-system - TTS}). Habituellement: 10, 15 ou 20 minutes en réseaux urbains et 30 minutes en réseaux interurbains/régionaux. \\
\hline
\end{longtable}
...
\hline
\makecell[r]{Capacité de ligne \\ \textit{Line capacity}} & \[C_L\] & \[places/h\] & \[\frac{c_u f_{up}}{\alpha_c} = \frac{P_d}{\alpha_c}\] & Capacité maximale offerte sur une ligne en pointe, dans une direction, en tenant compte du coefficient de confort. Cette capacité est \(\leq\) à la capacité théorique maximale qui ne tient pas compte du niveau de confort souhaité. \\
\hline
\makecell[r]{Coefficient de confort \\ \textit{Comfort coefficient}} & \[\alpha_c\] & - & - & Coefficient permettant de déterminer le niveau de confort requis lors de la conception et du choix de la capacité offerte. Si on veut éviter les passagers debout: \(\alpha_c = C_{{u}_{seat}} / C_{u}\). Relativement confortable: \(\alpha_c = 0.7\) \\
\hline
...
\hline
\makecell[r]{Coefficient d'utilisation de la capacité \\ \textit{Used capacity coefficient}} & \[\alpha_u\] & - & \[\frac{\sum_{i=1}^{n_l} {P_{l_i}}}{C_L}\] & Coefficient permettant d'évaluer le pourcentage d'utilisation de la capacité offerte sur une ligne, par heure. \\
\hline
...
\hline
\makecell[r]{Coût d'opération par unité-km \\ \textit{Operating cost per unit-km}} & \[c_{d_o}\] & \[\$/km\] & & Coût d'opération d'une unité par km. Attention d'inclure les distances des trajets haut-le-pied. \\
\hline
...
\hline
\makecell[r]{Facteur de pénalité du temps d'accès OD \\ \textit{Access/egress time penalty factor}} & \[\mu_{a_{OD}}\] & - & - & Facteur par lequel on multiplie le temps d'accès à l'origine et à destination. Varie en fonction de la perception de pénibilité associée aux temps d'accès à l'origine et à destination. \\
\hline
\makecell[r]{Facteur de pénalité du temps d'accès total \\ \textit{Total access time penalty factor}} & \[\mu_a\] & - & - & Facteur par lequel on multiplie le temps d'accès total. Varie en fonction de la perception de pénibilité associée aux temps d'accès total (à l'origine, à destination et lors des transferts). \\
\hline
...
\hline
\makecell[r]{Nombre de transferts \\ \textit{NUmber of transfers}} & \[n_{tr}\] & transferts & - & Nombre de transferts effectués par l'usager lors d'un trajet de transport collectif. \\
\hline
...
\hline
\makecell[r]{Nombre de trajets possibles \\ \textit{Number of possible user paths}} & \[N_{\Delta q}\] & trajets & \[\frac{1}{2} N_q (N_q-1)\] & Nombre total de paires de noeuds d'arrêts uniques dans le réseau. \\
\hline
\makecell[r]{Nombre de trajets directs possibles \\ \textit{Number of possible direct user paths}} & \[N_{{\Delta q}_d}\] & trajets & \[\frac{1}{2} \Bigg( \sum_{i=1}^{N_L} {n_{q_i}(n_{q_i} - 1)}\] \[-\sum_{j=1}^{N_{Lm}-1} {n_{q_{m_j}}(n_{q_{m_j}} - 1)} \Bigg)\] & Nombre total de paires de noeuds d'arrêts uniques dans le réseau pouvant être desservies sans transfert (directs). \(N_{L_{m}}\) est le nombre de lignes qui possèdent au moins un segment commun avec une autre ligne et \(n_{q_{m_j}}\) est le nombre de stations sur chaque segment que chaque ligne \(j\) partage avec une ligne déjà comptée. \\
\hline
\makecell[r]{Nombre de trajets avec transferts possibles \\ \textit{Number of possible user paths with transfers}} & \[N_{{\Delta q}_{tr}}\] & trajets & \[N_{\Delta q} - N_{{\Delta q}_d}\] & Nombre total de paires de noeuds d'arrêts uniques dans le réseau ne pouvant être desservies qu'avec au moins un transfert. \\
\hline
\makecell[r]{Coefficient de directivité des trajets \\ \textit{Direct user paths coefficient}} & \[\delta_{{\Delta q}_d}\] & - & \[\frac{N_{{\Delta q}_d}}{N_{\Delta q}}\] & Pourcentage des trajets possibles qui peuvent se faire directement, sans transfert. \\
\hline
